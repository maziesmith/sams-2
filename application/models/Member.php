...
class Member extends CI_Model {
    ...
    public function import($file=null, $truncate=false)
    {
        ...
        $query = "LOAD DATA local INFILE '".addslashes($file)."' INTO TABLE ".$this->pdo->dbprefix.$this->table." CHARACTER SET ".$this->pdo->char_set." FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n' IGNORE 1 LINES (id, @nick, stud_no, firstname, middlename, lastname, level, type, address_blockno, address_street, address_brgy, address_city, address_zip, telephone, @msisdn, email, groups, avatar, created_by, updated_by, removed_by, @created_at, @updated_at, @removed_at) SET created_at = STR_TO_DATE(@created_at, '%Y-%m-%d %H:%i:%s'), updated_at = STR_TO_DATE(@updated_at, '%Y-%m-%d %H:%i:%s'), removed_at = nullif(@removed_at, ''), nick=nullif(@nick,''), msisdn=LPAD(@msisdn, 11, '0')";
