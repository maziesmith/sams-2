jQuery(document).ready(function ($) {
    var membersTrashTable = $('#trashed-member-table-command').bootgrid({
        labels: {
        ...
        membersTrashTable.find('.command-restore').on('click', function (e) {
            var id   = $(this).parents('tr').data('row-id'),
                name = $(this).parents('tr').find('td.fullname').text(),
                url  = base_url('members/restore/' + id);
            e.preventDefault();
            $.ajax({
                type: 'POST',
                url: url,
...
function reload_trash_table() {
    $('#trashed-member-table-command').bootgrid('reload');
}